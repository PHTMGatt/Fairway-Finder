...
router.get(
  ...
  async (
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: 'Internal server error fetching weather' })
    }
  }
)
...
router.get(
  '/weather/forecast',
  async (
    req: Request<object, any, object, { city?: string; lat?: string; lon?: string }>,
    res: Response
  ) => {
    const city = req.query.city?.trim()
    ...
    if (!city && (!lat || !lon)) {
      return res.status(400).json({
        error: 'Query parameter "city" or both "lat" and "lon" are required',
      })
    }
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: 'Internal server error fetching forecast' })
    }
  }
)
