...
const CourseList: React.FC<CourseListProps> = ({ courses = [], isLoggedInUser }) => {
  ...
  if (!courses.length) return <h3 className="course-list__empty">No Courses Added Yet</h3>;
  ...
  return (
    <div className="course-list">
      {courses.map((course) => (
        <div key={course} className="course-card">
          <h4 className="course-card__name">
            ...
            {isLoggedInUser && (
              <button
                className="btn btn-sm btn-danger course-card__remove"
      ...
      {error && <p className="course-list__error">{error.message}</p>}
    </div>
  );
};
